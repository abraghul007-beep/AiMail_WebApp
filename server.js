...
const express=require('express');const session=require('express-session');const crypto=require('crypto');const {google}=require('googleapis');const path=require('path');
const app=express(),PORT=Number(process.env.PORT||3000),prod=process.env.NODE_ENV==='production';
...
const clients=new Map();app.disable('x-powered-by');app.set('trust proxy',1);app.use(express.json({limit:'256kb'}));app.use(session({name:'nebula.sid',secret:process.env.SESSION_SECRET||(prod?crypto.randomBytes(48).toString('hex'):'dev-only-change-me'),resave:false,saveUninitialized:false,cookie:{httpOnly:true,sameSite:'lax',secure:prod,maxAge:86400000}}));
const oauth=()=>new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID,process.env.GOOGLE_CLIENT_SECRET,process.env.GOOGLE_REDIRECT_URI||`http://localhost:${PORT}/auth/callback`);const auth=req=>{const c=oauth();c.setCredentials(req.session.tokens||{});return c};const gmail=req=>google.gmail({version:'v1',auth:auth(req)});const guard=(req,res,next)=>req.session.tokens?next():res.status(401).json({error:'AUTH_REQUIRED'});
const hdr=(hs,n)=>(hs||[]).find(h=>String(h.name).toLowerCase()===n.toLowerCase())?.value||'';const dec=s=>s?Buffer.from(s.replace(/-/g,'+').replace(/_/g,'/'),'base64').toString():'';
function body(p){if(!p)return'';if(p.mimeType==='text/plain'&&p.body?.data)return dec(p.body.data);for(const x of p.parts||[]){const b=body(x);if(b)return b}return''}function normalize(m){return{id:m.id,threadId:m.threadId,sender:hdr(m.payload?.headers,'From'),to:hdr(m.payload?.headers,'To'),cc:hdr(m.payload?.headers,'Cc'),subject:hdr(m.payload?.headers,'Subject')||'(no subject)',date:hdr(m.payload?.headers,'Date'),snippet:m.snippet||'',body:body(m.payload),unread:(m.labelIds||[]).includes('UNREAD'),labels:m.labelIds||[]}}
function buildQuery(q){const p=q?.query||q||{};return[p.q,p.from&&`from:${p.from}`,p.unread==='true'&&'is:unread',p.after&&`after:${p.after}`,p.before&&`before:${p.before}`].filter(Boolean).join(' ')}function listOptions(req){const label=String(req.query.label||'INBOX').toUpperCase(),o={userId:'me',q:buildQuery(req.query),maxResults:Math.min(Math.max(Number(req.query.maxResults||30),1),50)};if(req.query.pageToken)o.pageToken=String(req.query.pageToken);if(['INBOX','SENT','DRAFT','STARRED','SPAM','TRASH'].includes(label))o.labelIds=[label];if(label==='ARCHIVE')o.q=`${o.q} -label:inbox -label:trash -label:spam`.trim();return o}async function list(req){const s=gmail(req),r=await s.users.messages.list(listOptions(req)),ids=r.data.messages||[],ms=await Promise.all(ids.map(x=>s.users.messages.get({userId:'me',id:x.id,format:'full'})));return{messages:ms.map(x=>normalize(x.data)),nextPageToken:r.data.nextPageToken||null}}
app.get('/auth/url',(req,res)=>{const state=crypto.randomBytes(32).toString('hex');req.session.oauthState=state;res.json({url:oauth().generateAuthUrl({access_type:'offline',prompt:'consent',include_granted_scopes:true,scope:SCOPES,state})})});app.get('/auth/callback',async(req,res)=>{try{if(!req.query.code||req.query.state!==req.session.oauthState)return res.status(400).send('OAuth state validation failed.');delete req.session.oauthState;const{tokens}=await oauth().getToken(req.query.code);await new Promise((ok,bad)=>req.session.regenerate(e=>e?bad(e):ok()));req.session.tokens=tokens;res.redirect('/')}catch{res.status(400).send('OAuth failed. Please restart sign-in.')}});app.post('/auth/logout',(req,res)=>req.session.destroy(()=>res.json({ok:true})));
